...
public class CreaturePathNavigate extends PathNavigate {
    ...
    protected BlockPos getSuitableDestination(BlockPos pos) {
        IBlockState targetBlockState = this.worldObj.getBlockState(pos);
    ...
    protected int getPathablePosY() {
        ...
        if(this.entityCreature.isInWater()) {
            ...
            if(this.entityCreature.canFloat() && !this.entityCreature.canDive()) {
                int posY = (int)this.theEntity.getEntityBoundingBox().minY;
                Block block = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.theEntity.posX), posY, MathHelper.floor_double(this.theEntity.posZ))).getBlock();
                int searchCount = 0;

                while (this.isSwimmableBlock(block)) { // Search up for surface.
                    ++posY;
                    block = this.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(this.theEntity.posX), posY, MathHelper.floor_double(this.theEntity.posZ))).getBlock();
                    ++searchCount;
                    if (searchCount > 16) {
    ...
    public BlockPos getGround(BlockPos pos) {
        ...
        for(resultPos = pos.down(); resultPos.getY() > 0 && !this.worldObj.getBlockState(resultPos).getMaterial().isSolid(); resultPos = resultPos.down()) {}
        resultPos = resultPos.up();
        if(resultPos.getY() == 0 && !this.worldObj.getBlockState(resultPos).getMaterial().isSolid())
            return pos;
        return resultPos;
    }
    ...
    public BlockPos getSurface(BlockPos pos) {
        BlockPos resultPos = pos;
        for(resultPos = pos.up(); resultPos.getY() < this.worldObj.getHeight() && this.worldObj.getBlockState(resultPos).getMaterial().isSolid(); resultPos = resultPos.up()) {}
        if(resultPos.getY() == this.worldObj.getHeight() && this.worldObj.getBlockState(resultPos).getMaterial().isSolid())
            return pos;
        return resultPos;
    }
    ...
    protected boolean isDirectPathBetweenPoints(Vec3d startVec, Vec3d endVec, int sizeX, int sizeY, int sizeZ) {
        ...
        int i = MathHelper.floor_double(startVec.xCoord);
        int j = MathHelper.floor_double(startVec.zCoord);
        double distanceX = endVec.xCoord - startVec.xCoord;
        double distanceY = endVec.zCoord - startVec.zCoord;
        ...
        if (distance < 1.0E-8D) {
            ...
            if(!this.isSafeToStandAt(i, (int)startVec.yCoord, j, sizeX, sizeY, sizeZ, startVec, distanceX, distanceY)) {
                return false;
            }
            else {
                sizeX = sizeX - 2;
                ...
                double d6 = (double)i - startVec.xCoord;
                double d7 = (double)j - startVec.zCoord;
                ...
                int i1 = MathHelper.floor_double(endVec.xCoord);
                int j1 = MathHelper.floor_double(endVec.zCoord);
                int k1 = i1 - i;
                int l1 = j1 - j;
        ...
        if(!this.isPositionClear(i, y, j, sizeX, sizeY, sizeZ, startVec, distanceX, distanceZ)) {
            ...
            for(int k = i; k < i + sizeX; ++k) {
                for (int l = j; l < j + sizeZ; ++l) {
                    double d0 = (double)k + 0.5D - startVec.xCoord;
                    double d1 = (double)l + 0.5D - startVec.zCoord;
    ...
    private boolean isPositionClear(int x, int y, int z, int sizeX, int sizeY, int sizeZ, Vec3d startVec, double distanceX, double distanceZ) {
        for (BlockPos blockpos : BlockPos.getAllInBox(new BlockPos(x, y, z), new BlockPos(x + sizeX - 1, y + sizeY - 1, z + sizeZ - 1))) {
            double d0 = (double)blockpos.getX() + 0.5D - startVec.xCoord;
            double d1 = (double)blockpos.getZ() + 0.5D - startVec.zCoord;
