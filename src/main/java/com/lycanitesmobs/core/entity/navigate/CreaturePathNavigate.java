...
public class CreaturePathNavigate extends PathNavigate {
    ...
    protected int getPathablePosY() {
        ...
        if(this.entityCreature.isInWater()) {
            ...
            if(this.entityCreature.canFloat() && !this.entityCreature.canDive()) {
                ...
                Block block = this.world.getBlockState(new BlockPos(MathHelper.floor(this.theEntity.posX), posY, MathHelper.floor(this.theEntity.posZ))).getBlock();
                int searchCount = 0;

                while (this.isSwimmableBlock(block)) { // Search up for surface.
                    ++posY;
                    block = this.world.getBlockState(new BlockPos(MathHelper.floor(this.theEntity.posX), posY, MathHelper.floor(this.theEntity.posZ))).getBlock();
                    ++searchCount;
                    if (searchCount > 16) {
    ...
    protected boolean isDirectPathBetweenPoints(Vec3d startVec, Vec3d endVec, int sizeX, int sizeY, int sizeZ) {
        // Flight/Swimming:
        if(this.entityCreature.isFlying() || this.entityCreature.isInWater()) {
            RayTraceResult raytraceresult = this.world.rayTraceBlocks(startVec, new Vec3d(endVec.xCoord, endVec.yCoord + (double)this.theEntity.height * 0.5D, endVec.zCoord), false, true, false);
            return raytraceresult == null || raytraceresult.typeOfHit == RayTraceResult.Type.MISS;
        }

        int i = MathHelper.floor(startVec.xCoord);
        int j = MathHelper.floor(startVec.zCoord);
        ...
        if (distance < 1.0E-8D) {
            ...
            if(!this.isSafeToStandAt(i, (int)startVec.yCoord, j, sizeX, sizeY, sizeZ, startVec, distanceX, distanceY)) {
                ...
                int i1 = MathHelper.floor(endVec.xCoord);
                int j1 = MathHelper.floor(endVec.zCoord);
                int k1 = i1 - i;
                int l1 = j1 - j;
